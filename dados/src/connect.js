...
async function createBotSocket(authDir) {
    try {
        ...
        if (codeMode && !NazunaSock.authState.creds.registered) {
            console.log('📱 Insira o número de telefone (com código de país, ex: +258852931823 ou +258852931823): ');
            let phoneNumber = await ask('--> ');
            phoneNumber = phoneNumber.replace(/\D/g, '');
            if (!/^\d{10,15}$/.test(phoneNumber)) {
                console.log('⚠️ Número inválido! Use um número válido com código de país (ex: 258852931823).');
                process.exit(1);
            }
...
async function startNazu() {
    ...
    try {
        console.log('🚀 Iniciando Gojo Service...');
